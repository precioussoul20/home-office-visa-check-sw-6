...
public class VisaConfirmationSteps {
    ...
    public void verifyResultYouWillNotNeedAVisaToComeToTheUK() {
        Assert.assertEquals(new ResultPage().getResultMessageTourism(), "You will not need a visa to come to the UK", "Error");
    }
    ...
    public void verifyResultOfYouNeedAVisaToWorkInHealthAndCare() {
        Assert.assertEquals(new ResultPage().getResultMessageHealthcare(), "You need a visa to work in health and care", "Error");
    }
    ...
    public void toGetVerifyResultYouMayNeedAVisaTpJoinYourFamilyOrPartnerInTheUK() {
        Assert.assertEquals(new ResultPage().getResultMessageFamilyStay(),"You’ll need a visa to join your family or partner in the UK","Error");
    }
}
